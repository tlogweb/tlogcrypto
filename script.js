/*
  tlog-crypto.js
  - 코인 매매일지 전용 로직
  - 로컬스토리지 저장/로드, 차트/통계 계산, 테이블 렌더링, 모달(상세 보기/수정),
    CSV 내보내기/가져오기, 날짜 픽커 자동 오픈 등 모든 동작 관리
*/
...
const LEGACY_SETTINGS_KEY = "tradeJournalSettings_v1"; // 있으면 1회만
...
function loadPrinciples() {
  try {
    ...
    if (stored !== null) {
      ...
      principlesStatus.textContent = "아직 저장된 매매 원칙이 없습니다.";
    }
  } catch (e) {
    console.error(e);
...
/* ===== 그래프/통계 =====
   선택범위 시작일 '직전까지'의 누적손익을 베이스라인으로 하여
   시작일~현재까지의 손익을 누적하여 선형으로 그림
*/
...
chartCanvas.addEventListener("mousemove", (e) => {
  ...
  const { padL, padR, innerW, points, minV, span, h, padB, innerH } =
    lastChartGeom;
  ...
  // === 여기서부터 위치 계산 로직 (차트 가운데 기준) ===
  const centerX = rect.width / 2;              // 차트 가로 중앙 (캔버스 좌표)
  const tooltipWidth = chartTooltip.offsetWidth || 150;

  let tooltipX;
  if (x > centerX) {
    // 마우스가 차트 오른쪽 절반에 있으면 → 툴팁을 왼쪽에 표시
    tooltipX = px - tooltipWidth - 12;
  } else {
    // 왼쪽 절반에 있으면 → 툴팁을 오른쪽에 표시
    tooltipX = px + 12;
  }

  // 화면 밖으로 너무 나가는 것 최소한 방지 (옵션)
  tooltipX = Math.max(0, Math.min(tooltipX, rect.width - tooltipWidth));

  chartTooltip.style.left = tooltipX + "px";
  chartTooltip.style.top = py - 10 + "px";
});
...
function renderTodayStats() {
  ...
  dates.forEach((d) => {
    cum += dailyPnl[d];
    equityByDate[d] = startCash + cum; // 해당 날짜까지 누적 평가금
  });

  const todayPnl = dailyPnl[today] || 0;
  statTodayPnl.textContent = fmtInt(todayPnl);

  // 이전 거래일(오늘보다 작은 날짜 중 가장 최근)을 찾기
...
function renderPositionStats() {
  ...
  entries.forEach((e) => {
    ...
    const isWin = pnl > 0;
    if ((e.side || "LONG").toUpperCase() === "SHORT") {
      isWin ? s.short.win++ : s.short.lose++;
    } else {
      isWin ? s.long.win++ : s.long.lose++;
    }
    isWin ? s.total.win++ : s.total.lose++;
  });
  const totalCount = s.total.win + s.total.lose;
  const longCount = s.long.win + s.long.lose;
  const shortCount = s.short.win + s.short.lose;
  const pct = (w, t) => (t ? ((w / t) * 100).toFixed(2) + "%" : "-");

  posOverallMain.textContent = `WIN ${s.total.win} / LOSE ${s.total.lose}`;
  posOverallSub.textContent = `승률 ${pct(
    s.total.win,
    totalCount
  )} (총 ${totalCount})`;

  posLongMain.textContent = `WIN ${s.long.win} / LOSE ${s.long.lose}`;
  posLongSub.textContent = `승률 ${pct(
    s.long.win,
    longCount
  )} (총 ${longCount})`;

  posShortMain.textContent = `WIN ${s.short.win} / LOSE ${s.short.lose}`;
  posShortSub.textContent = `승률 ${pct(
    s.short.win,
    shortCount
  )} (총 ${shortCount})`;
}
...
function setModalReadOnly(ro) {
  ...
  modalEdit.style.display = ro ? "" : "none";
  modalSave.style.display = ro ? "none" : "";
  modalCancel.style.display = ro ? "none" : "";
}
...
exportBtn.addEventListener("click", () => {
  ...
  a.href = url;
  a.download = `coin_journal_${todayISO()}.csv`;
...
csvFileInput.addEventListener("change", () => {
  ...
  reader.onload = (ev) => {
    try {
      ...
      console.error(err);
      alert("CSV 파싱 중 오류가 발생했습니다. 파일 형식을 확인해 주세요.");
    } finally {
      csvFileInput.value = "";
    }
  };
